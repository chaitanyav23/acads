...
library(ggplot2)
library(reshape2)
ggplot(melt(subset(chains,sample<100),id="sample"),aes(x=sample,y=value,group=variable,color=variable))+
  geom_line()
...
library(ggplot2)
ggplot(df.post,aes(x=post))+geom_density(adjust=1.5)+geom_density(aes(x=analytical),color="red",adjust=1.5)
...
mcmc <- function(y,step_size){
  ...
  while(i < nsamp){
    ...
    if(astr<min(1,H)){
      lambda_chain[i+1] <- proposal_lambda
      i <- i+1
      print(paste(as.character(i),"/",as.character(nsamp)," collected"))
    }else{
      reject <- reject+1
    }
  }
...
chain1 <- mcmc(y=y,step_size = 0.0008)
chain2 <- mcmc(y=y,step_size = 0.0008)
chain3 <- mcmc(y=y,step_size = 0.0008)
chain4 <- mcmc(y=y,step_size = 0.0008)
